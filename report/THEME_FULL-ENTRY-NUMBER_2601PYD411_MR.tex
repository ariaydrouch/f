...
\titlespacing*{name=\section,numberless}{0pt}{5pt}{3pt}
...
be predicted from a few mechanical parameters~\cite{Keren:2008}, which
motivates minimal physical models such as that of Tjhung, Tiribocchi,
Marenduzzo and Cates~\cite{Tjhung:2015}, who treat a crawling cell as a
droplet of active polar fluid. Averaged over stochastic mechanosensitive
linkers, substrate adhesion reduces to an effective
friction~\cite{Leoni:2017,Ziebert:2013}.

The long-term goal of this project is many interacting cells. The Stokes
solve of Ref.~\cite{Tjhung:2015} is elliptic, hence expensive for one cell
and prohibitive for many. The mid-term work therefore implements the full
model, identifies which ingredients actually matter, and constructs a
cheaper model whose accuracy we can quantify (Table~\ref{tab:versions}).
...
neighbouring actin; and anchoring $\beta>0$ points $\mathbf{P}$ outward,
normal to the boundary, and is the controlling shape parameter. Transport
is by $\mathbf{u}=\mathbf{v}+w_{0}\mathbf{P}$, with $w_{0}$ the treadmilling
...
$g=\phi(1-\phi)$, and $\mathbf{D}$, $\boldsymbol{\Omega}$ are the strain-rate
and vorticity. At Reynolds number $\sim10^{-5}$ the force balance is
Stokes; in a top view the wall becomes a depth-averaged friction
...
\begin{figure}[H]
\centering
\includegraphics[width=13.2cm,height=3.55cm,keepaspectratio]{figures/fig_v2_shapes.pdf}\\[2pt]
\includegraphics[width=13.2cm,height=2.15cm,keepaspectratio]{figures/fig_v2_transition.pdf}
...
speed is zero to machine precision, the non-motile fried egg of
Ref.~\cite{Tjhung:2015} and of experiment~\cite{Yam:2007}. Moderate
anchoring instead gives a fan elongated \textit{across} the path (aspect
$2.91$, speed $0.843\approx w_{0}$), the keratocyte lamellipodium, and
driving anchoring harder gives a more extreme fan (aspect $4.35$). A plane
pseudopod cannot form (contractility only widens the cell), and the
phagocytic cup is three-dimensional; Version~2 already captures the
vertical mechanism of the former.

\begin{figure}[H]
\centering
\includegraphics[width=13.2cm,height=4.65cm,keepaspectratio]{figures/fig_v3_morph.pdf}\\[2pt]
\includegraphics[width=13.2cm,height=2.30cm,keepaspectratio]{figures/fig_v3_decomp.pdf}
...
mechanisms: neither gives a disc of aspect $1.07$; flow alone $1.61$;
anchoring alone $2.85$; both together $2.92$ ($\approx2\%$ above anchoring
alone). Speed is $0.80$--$0.85$ throughout, set by
$w_{0}\langle\mathbf{P}\rangle$ and almost independent of the friction
$\xi$ --- unlike the biphasic speed of Ref.~\cite{Leoni:2017}, whose
mechanism needs mechanosensitive bonds that a constant $\xi$ cannot see.
...
(vorticity and flow alignment change the shape by $<1\%$), with
$\zeta=-0.25$ recalibrated for the missing pressure.
...
straight line, with $y_{\mathrm{cm}}=0$, constant speed, aspect ratio
saturating near $3.2$, and $A/A_{0}$ growing by $\approx15\%$ while the
implied thickness $h=V_{0}/A$ falls by the same factor.

\begin{figure}[H]
\centering
\includegraphics[width=13.4cm,height=2.5cm,keepaspectratio]{figures/fig_v4_montage.pdf}\\[1pt]
\includegraphics[width=13.4cm,height=2.5cm,keepaspectratio]{figures/fig_v4_snapshots.pdf}\\[1pt]
\includegraphics[width=13.4cm,height=1.9cm,keepaspectratio]{figures/fig_v4_diagnostics.pdf}
...
$35\%$; removing the constraint ($k_{A}=0$) inflates the cell $14$-fold and
stops crawling. Cost falls from $2.70$ to $1.10\,\mathrm{ms}$/step and the
stable $dt$ rises from $0.01$ to $0.03$, about $5\times$ overall.
...
\noindent
The four stages are verified and volume is conserved to machine precision
...
lamellipodial fan, with shape anchoring-driven and flow adding only
$+2\%$ (Fig.~\ref{fig:v3}); and Version~4 crawls at speed $0.801$
(Fig.~\ref{fig:v4}), $\approx5\times$ faster, agreeing to $0.2\%$ where
comparable, and local. Soft volume lets the cell spread and thin;
